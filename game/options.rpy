...
init -1 python hide:
    ...
    config.window_title = u"gungrave_project"

    # 트레이스백 파일과 기타 디버그 기록에 적힐
    # 게임 이름과 버전을 입력합니다.
    config.name = "gungrave_project"
    config.version = "0.0"
...
python early:
    config.save_directory = "gungrave_project-1485450803"
...
init python:
    ...
    build.directory_name = "gungrave_project-1.0"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "gungrave_project"
